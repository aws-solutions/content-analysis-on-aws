...
def start_content_moderation(bucket, key):
    ...
    try:
        ...
        print('Job Id (content moderation): ' + response['JobId'])
...
def lambda_handler(event, context):
    ...
    try:
        ...
        output_object.update_workflow_status("Error")
        output_object.add_workflow_metadata(ContentModerationError="No valid inputs")
        raise MasExecutionError(output_object.return_output_object())
    ...
    if file_type in valid_image_types:
        ...
        response = detect_moderation_labels(s3bucket, s3key)
        ...
        job_id = start_content_moderation(s3bucket, s3key)
        ...
        output_object.update_workflow_status("Error")
        output_object.add_workflow_metadata(ContentModerationError="Not a valid file type")
        raise MasExecutionError(output_object.return_output_object())
