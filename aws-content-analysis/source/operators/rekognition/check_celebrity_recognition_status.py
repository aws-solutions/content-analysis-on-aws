...
def lambda_handler(event, context):
    ...
    try:
        ...
        output_object.add_workflow_metadata(CelebrityRecognitionError="Missing key {e}".format(e=e))
    ...
    try:
        ...
        output_object.update_workflow_status("Error")
        output_object.add_workflow_metadata(CelebrityRecognitionError="Missing a required metadata key {e}".format(e=e))
        raise MasExecutionError(output_object.return_output_object())
    ...
    for page_number in range(11):
        ...
        try:
            response = rek.get_celebrity_recognition(JobId=job_id, NextToken=pagination_token)
        ...
        # If the reko job is FAILED then mark the workflow status as Error and return.
            ...
            output_object.update_workflow_status("Error")
            output_object.add_workflow_metadata(JobId=job_id, CelebrityRecognitionError=str(response["StatusMessage"]))
            raise MasExecutionError(output_object.return_output_object())
        ...
        # and continue to next page_number.
            ...
            if 'NextToken' in response:
                ...
                # If dataplane request failed then mark workflow as failed
                    ...
                    output_object.update_workflow_status("Error")
                    output_object.add_workflow_metadata(CelebrityRecognitionError="Unable to upload metadata for asset: {asset}".format(asset=asset_id), JobId=job_id)
                    raise MasExecutionError(output_object.return_output_object())
            # If reko results contain no more pages then save this page and mark the stage complete
                ...
                # If dataplane request failed then mark workflow as failed
                    ...
                    output_object.update_workflow_status("Error")
                    output_object.add_workflow_metadata(CelebrityRecognitionError="Unable to upload metadata for {asset}: {error}".format(asset=asset_id, error=metadata_upload))
                    output_object.add_workflow_metadata(JobId=job_id)
                    raise MasExecutionError(output_object.return_output_object())
        # If reko job failed then mark workflow as failed
        else:
            output_object.update_workflow_status("Error")
            output_object.add_workflow_metadata(CelebrityRecognitionError="Unable to determine status")
            raise MasExecutionError(output_object.return_output_object())
