  ...
  export default {
    ...
    methods: {
      ...
      async getVideoUrl() {
        ...
        let key = this.s3_uri.split(this.s3_uri.split("/")[2] + '/')[1];
        // Load proxy video file for video media types
        if (this.mediaType === "video") {
          key = key.substr(0, key.lastIndexOf(".")).replace('/input', '') + "_proxy.mp4";
        }
        ...
        fetch(this.DATAPLANE_API_ENDPOINT + '/download', {
          ...
          body: JSON.stringify({"S3Bucket": bucket, "S3Key": key})
            ...
            console.log({"S3Bucket": bucket, "S3Key": key})
